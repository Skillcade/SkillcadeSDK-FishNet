    ...
    public class UnityTransport : Transport, INetworkStreamDriverConstructor
    {
        ...
        private void ReceiveMessages(ulong clientId, NetworkPipeline pipeline, DataStreamReader dataReader)
        {
            ...
            while (!queue.IsEmpty)
            {
                ...
                HandleTransportDataEvent(clientId, message, pipeline);
            }
        }
        ...
        private bool ProcessEvent()
        {
            ...
            switch (eventType)
            {
                case TransportNetworkEvent.Type.Connect:
                    {
                        HandleTransportConnectEvent(clientId);
        ...
        private bool DisconnectRemoteClient(ulong clientId)
        {
            ...
            if (m_State == State.Listening)
            {
                ...
                if (m_Driver.GetConnectionState(connection) != NetworkConnection.State.Disconnected)
                {
                    ...
                    // Disconnect doesn't notify SDK of disconnection.
                    HandleTransportDisconnectEvent(clientId);
                    return true;
                }
            }
        ...
        private bool StartServer()
        {
            ...
            else
            {
                ...
                StopClientHost(); // Fail client host if starting
                DisposeInternals();
        ...
        {
            var clientSeverOrHost = m_ServerState.IsStartingOrStarted() ? m_ClientState.IsStartingOrStarted() ? "Host" : "Server" : "Client";
            bool duringStart = m_ServerState == LocalConnectionState.Starting || m_ClientState == LocalConnectionState.Starting;
            var whenFailed = duringStart ? "start failure" : "failure";
            NetworkManager.LogError($"{clientSeverOrHost} is shutting down due to network transport {whenFailed} of {nameof(UnityTransport)}!");
            Shutdown();
        }
