...
namespace FishNet.Transporting.Bayou.Server
{
    public class ServerSocket : CommonSocket
    {
        ...
        private void Socket()
        {
            ...
            _server = new SimpleWebServer(5000, tcpConfig, _mtu, 5000, config, 2048);
        ...
        /// </summary>
        private void _server_onError(IConnection connection, Exception arg2)
        {
            ConnectionStoppedOnSocket(connection.Id);
        }
        ...
        /// </summary>
        private void _server_onData(IConnection connection, ArraySegment<byte> data)
        {
            ...
            ServerReceivedDataArgs dataArgs = new ServerReceivedDataArgs(segment, channel, connection.Id, base.Transport.Index);
        ...
        /// </summary>
        private void _server_onConnect(IConnection connection)
        {
            if (_server == null || !_server.Active)
                return;
            ...
            {
                _server.KickClient(connection.Id);
                return;
            }

            _clients.Add(connection.Id);
            RemoteConnectionState state = RemoteConnectionState.Started;
            base.Transport.HandleRemoteConnectionState(new RemoteConnectionStateArgs(state, connection.Id, base.Transport.Index));
        }

        /// <summary>
        /// Called when a client disconnects.
        /// </summary>
        private void _server_onDisconnect(IConnection connection)
        {
            ConnectionStoppedOnSocket(connection.Id);
        }
